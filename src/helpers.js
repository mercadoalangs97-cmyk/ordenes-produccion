...
export async function generateContract(order, geminiKey) {
  ...
  const prompt = [
    "Genera un contrato de compraventa profesional en español para artículos promocionales.",
    "VENDEDOR: Promocionales GASCA",
    "CLIENTE: "+(order.cliente||""), "EMPRESA: "+(order.empresa||""),
    "TELÉFONO: "+(order.telefono||""), "EMAIL: "+(order.email||""),
    "FECHA: "+(order.fecha||""), "FOLIO: "+(order.folio||""),
    "PRODUCTOS:", prods,
    "PAGOS:", "- Anticipo: "+anticipo, "- Finiquito: "+finiquito,
    "Incluir: datos de las partes, productos y servicios, condiciones de pago, no devoluciones, propiedad intelectual, firmas.",
  ].join("\n");
